...
ll startel[200005],endel[200005];

bool isvalid(ll mid,ll n){
  ll currmini=0,currmaxi=0;
  for(int i=0;i<n;i++){
    currmini-=mid;
    currmaxi+=mid;
    ll start=max(currmini,startel[i]);
    ll end=min(currmaxi,endel[i]);
