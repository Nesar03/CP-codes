...
ll last=(h-1)/sum;
ll time=n*last + k*last;
h-=last*sum;
if(h<=0){
  cout<<time<<endl;
  return;
}
vector<ll> maxelright(n);
maxelright[n-1]=v[n-1];
for(int i=n-2;i>=0;i--){
  maxelright[i]=max(maxelright[i+1],v[i]);
}
ll minel=v[0];
for(int i=0;i<n;i++){
minel=min(minel,v[i]);
ll currsum=pref[i]-minel;
if(i+1<n) currsum+=maxelright[i+1];
if(h<=currsum || h<=pref[i]){
  cout<<time+i+1<<endl;
  return;
}
}
