...
bool isvalid(ll mid, int k, int l, vector<vector<ll>>& pref){
  ll currval=0;
  for(ll i=31;i>=0;i--){
    ll count=pref[i][mid]-pref[i][l-1];
    if(count==(mid-l+1)){
      currval |=(1LL<<i);
    }
    }
...
vector<vector<ll>> pref(32,vector<ll>((n+1),0));
for(int i=0;i<32;i++){
for(int j=1;j<=n;j++){
  pref[i][j]=pref[i][j-1];
  if(v[j] & (1LL<<i)) pref[i][j]++;
}
}
